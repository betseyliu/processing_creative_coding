...
class Point {
  ...
  public void rotate(Point p, float a) {
    float dx = x - p.x;
    float dy = y - p.y;
...
// 位于一个圆内的点
public void insideCirclePoints() {
  VisualPoint[] points = new VisualPoint[POINT_NUMBER];
  
  for(int i = 0; i < POINT_NUMBER; i++) {
    float radian = random(TAU);
    // float radius = random(450);
    float radius = pow(random(1), .1f) * 450;
...
public boolean isPrime(int number) {
  for(int i = 2; i <= PApplet.parseInt(number / 2 + 1); i++) {
...
// 圆内部均匀分布的点
public void evenlyDistributedPointsInCircle() {
  ...
  while(radius < 450) {
    ...
    int n = PApplet.parseInt(TAU * radius / arc);
    radian += TAU / n;
    count++;
    index++;
    if(count == n) {
...
public void linkRotatePoint() {
  ...
  for(int t = 0; t < 20; t++) {
    ...
    for(int i = 0; i < points.length; i++) {
      ...
      points[i].rotate(points[index], 0.01f);
    }
    // 距离规则
    for(int i = 0; i < points.length; i++) {
      Point a = points[i];
